...
export default function TaskFlow({ session }) {
  ...
  const handleDrop = useCallback(async (targetId) => {
    ...
    // Reorder in the active tasks list
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', -apple-system, sans-serif", height: "100vh", display: "flex", background: "#fdfdfc", color: "#1a1a1a", position: "relative" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .check { appearance: none; -webkit-appearance: none; width: 18px; height: 18px; border: 1.5px solid #c8c4be; border-radius: 4px; cursor: pointer; transition: all 0.15s; flex-shrink: 0; position: relative; background: #fff; }
        .check:checked { background: #b8b3ab; border-color: #b8b3ab; }
        .check:checked::after { content: '✓'; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 11px; color: #fff; font-weight: 600; }
        ...
        .modal { background: #fff; border-radius: 14px; box-shadow: 0 12px 48px rgba(0,0,0,0.12); width: 360px; max-height: 70vh; overflow: hidden; animation: slideUp 0.2s ease; }
      `}</style>

      {/* Sidebar */}
      <div style={{ width: 240, minWidth: 240, borderRight: "1px solid #eae8e4", display: "flex", flexDirection: "column", background: "#fdfdfc" }}>
          ...
          {[...activeGroups].sort((a, b) => a.name.localeCompare(b.name)).map(g => {
            ...
            return (
              <div key={g.id} className="g-item"
                onClick={() => { setSelected(g.id); setAddingTask(false); setExpandedTask(null); setContextMenu(null); }}
                ...
                style={{ padding: "10px 12px", display: "flex", alignItems: "center", background: selected === g.id ? "#f0eeea" : "transparent", marginBottom: 1 }}>
                <span style={{ fontSize: 13.5, fontWeight: selected === g.id ? 500 : 400, color: "#1a1a1a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1 }}>{g.name}</span>
                {count > 0 && <span style={{ fontSize: 11.5, color: "#b0aca6", marginLeft: 8, flexShrink: 0 }}>{count}</span>}
              </div>
            );
          })}
          {addingGroup ? (
            <div style={{ padding: "6px 12px", animation: "appear 0.15s ease" }}>
              <input ref={groupInputRef} value={newGroupName} onChange={e => setNewGroupName(e.target.value)}
                ...
                placeholder="Name..." style={{ width: "100%", padding: "8px 10px", borderRadius: 6, border: "1px solid #ddd9d3", fontSize: 13, outline: "none", background: "#fff" }} />
            </div>
          ) : (
            <button onClick={() => setAddingGroup(true)}
              style={{ display: "block", width: "calc(100% - 8px)", margin: "8px 4px", padding: "9px 12px", borderRadius: 6, border: "none", background: "transparent", color: "#b0aca6", fontSize: 13, textAlign: "left" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        {!selected ? (
          ...
          <>
            <div style={{ padding: "32px 40px 20px", borderBottom: "1px solid #eae8e4" }}>
              <h2 style={{ fontSize: 22, fontWeight: 400, letterSpacing: "-0.02em" }}>{selectedGroup?.name}</h2>
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 40px 40px" }}>
              ...
              {addingTask ? (
                <div style={{ padding: "10px 0 10px 42px", animation: "appear 0.15s ease" }}>
                  <input ref={taskInputRef} value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)}
                    ...
                    style={{ width: "100%", padding: "6px 0", border: "none", borderBottom: "1px solid #eae8e4", fontSize: 14, outline: "none", background: "transparent", color: "#1a1a1a" }} />
                </div>
              ) : (
                <button onClick={() => setAddingTask(true)}
                  style={{ padding: "10px 0 10px 42px", background: "none", border: "none", color: "#b0aca6", fontSize: 13.5, display: "block" }}>
                  + Add task
                </button>
              )}
...
function TaskRow({ task, expanded, draggable, isDragging, isDragOver, onDragStart, onDragOver, onDragEnd, onDrop, onToggle, onExpand, onUpdate, onRemove }) {
  ...
  return (
    <div
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 0, padding: "10px 8px 10px 0" }}>
        {draggable ? (
        ...
        <div style={{ flex: 1, minWidth: 0, marginLeft: 12 }}>
          {editingTitle ? (
            <input ref={titleRef} value={localTitle} onChange={e => setLocalTitle(e.target.value)}
              ...
              style={{ width: "100%", border: "none", borderBottom: "1px solid #ddd9d3", fontSize: 14, outline: "none", background: "transparent", padding: "0 0 2px", color: titleColor }} />
          ) : (
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span onClick={() => !task.done && setEditingTitle(true)}
                style={{ fontSize: 14, fontWeight: 400, color: titleColor, textDecoration: task.done ? "line-through" : "none", cursor: task.done ? "default" : "text", lineHeight: 1.45 }}>
                {task.title}
              </span>
              {hasDueDate && !task.done && !expanded && (
                <span style={{ fontSize: 11, flexShrink: 0, fontWeight: 500, color: isUrgent ? "#c4453a" : "#b0aca6" }}>
                  {isOverdue ? "overdue" : `due ${formatDate(dueDate)}`}
                </span>
              )}
              {isScheduled && !expanded && (
                <span style={{ fontSize: 11, color: "#b0aca6", flexShrink: 0 }}>{formatDate(activateDate)}</span>
              )}
              <button onClick={onExpand}
                style={{ background: "none", border: "none", padding: "1px 4px", color: hasDetails ? "#a8a4a0" : "#d4d1cc", fontSize: 13, lineHeight: 1, flexShrink: 0, borderRadius: 3 }}
                title="Details">
                {expanded ? "▾" : hasDetails ? "▸" : "＋"}
              </button>
            </div>
            ...
            <div style={{ marginTop: 10, animation: "appear 0.12s ease", display: "flex", flexDirection: "column", gap: 8 }}>
              {editingNotes ? (
                <textarea ref={noteRef} value={task.notes || ""} onChange={e => onUpdate({ notes: e.target.value })}
                  ...
                  style={{ width: "100%", padding: "8px 10px", borderRadius: 5, border: "1px solid #ddd9d3", fontSize: 13, lineHeight: 1.55, outline: "none", background: "#fafaf8", color: "#4a4a46" }} />
              ) : (
                <div onClick={() => setEditingNotes(true)}
                  style={{ padding: "8px 10px", borderRadius: 5, background: "#fafaf8", fontSize: 13, lineHeight: 1.55, color: hasNotes ? "#5a5a56" : "#b0aca6", cursor: "text", minHeight: 36, whiteSpace: "pre-wrap", borderLeft: hasNotes ? "2px solid #e0ddd8" : "2px solid transparent" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "2px 0" }}>
                  ...
                  <input type="date" value={dueDate}
                    ...
                    style={{ padding: "4px 8px", borderRadius: 5, border: `1px solid ${isUrgent ? "#e8c4c0" : "#e8e6e2"}`, fontSize: 13, color: isUrgent ? "#c4453a" : hasDueDate ? "#4a4a46" : "#b0aca6", outline: "none", background: isUrgent ? "#fdf6f5" : "#fafaf8" }} />
                  {hasDueDate && (
                    <button onClick={() => onUpdate({ dueDate: "" })}
                      style={{ background: "none", border: "none", fontSize: 13, color: "#c8c4be", padding: "2px 4px", lineHeight: 1 }}>×</button>
                  )}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "2px 0" }}>
                  ...
                  <input type="date" value={activateDate}
                    ...
                    style={{ padding: "4px 8px", borderRadius: 5, border: "1px solid #e8e6e2", fontSize: 13, color: hasActivateDate ? "#4a4a46" : "#b0aca6", outline: "none", background: "#fafaf8" }} />
                  {hasActivateDate && (
